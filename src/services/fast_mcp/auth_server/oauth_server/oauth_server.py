...
class OAuthServer:
    ...
    def create_server(self) -> Starlette:
    ...
    def run(self):
        async def run_server():
            ...
            auth_server = self.create_server(self.server_settings, self.auth_settings)
            ...
            logger.info(
                f"🚀 MCP Authorization Server running on {self.server_settings.server_url}"
            )
...
def main(port: int) -> int:
    ...
    server = OAuthServer(server_settings=server_settings)
